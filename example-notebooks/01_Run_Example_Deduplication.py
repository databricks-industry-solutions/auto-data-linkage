...
# MAGIC
# MAGIC We will union these two files together, and then use ARC to find the near duplicates we have introduced.
...
# MAGIC
# MAGIC ARC can work with only a single argument - the data! For deduplication, ARC requires a single Spark DataFrame. In most cases, you aren't going to have a a column like *recid* which you can use to tell you which records are duplicates - if you did, you wouldn't need to deduplicate your data! 
# MAGIC
# MAGIC For the first example, we will drop the *uid* and *recid* columns. This will simulate a dataset which doesn't have a primary key or a True record id.
# MAGIC
# MAGIC **NB** - ARC is an interative process, but for the sake of demonstration here we will only run a single iteration. This is controlled by the *max_evals* argument (this has a default of 5. In testing, we found good performance with 100 iterations).
# MAGIC
...
predictions = autolinker.best_predictions_df.as_spark_dataframe()
predictions.display()

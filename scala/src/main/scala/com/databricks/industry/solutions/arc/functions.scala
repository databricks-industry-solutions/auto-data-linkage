...
object functions {
    ...
    def arc_combinatorial_count_agg(nCombination: Int, threshold:Int, cols: String*): Column = {
        val exprs = cols.map(cn => new Column(cn).expr)
        new Column(ARC_CombinatorialCountAgg(exprs, cols, nCombination, threshold).toAggregateExpression())
    }

    def arc_combinatorial_count_agg(nCombination: Int, cols: java.util.ArrayList[String], threshold:Int): Column = {
        arc_combinatorial_count_agg(nCombination, threshold, cols.asScala: _*)
    }

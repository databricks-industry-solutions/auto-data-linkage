...
#     _udf_entropy = F.udf(lambda x: self._calculate_p(x), DoubleType())
  ...
  def deduplicate_records(self, data, predictions, linker, attribute_columns, unique_id, threshold=0.9):
    ...
    return df_deduped
  def deduplicate_records_new(self, data, predictions, linker, attribute_columns, unique_id, threshold=0.9):
    """
...
autolinker = AutoLinker(
  schema="autosplink" # optional arguments to determine where internal tables are stored
  , experiment_name="/Users/robert.whiffin@databricks.com/legacy_utosplink_with_group_dedupe"
#  , experiment_name="/Users/robert.whiffin@databricks.com/legacy_utosplink_with_window_dedupe"
)
...
autolinker.auto_link(
  ...
  comparison_size_limit=150000,                                      # Maximum number of pairs when blocking applied
  max_evals=50                                                      # Maximum number of hyperopt trials to run
)
